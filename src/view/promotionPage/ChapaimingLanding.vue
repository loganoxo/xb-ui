<template>
  <div class="chapaiming-landing">
      ...
      <div class="content">
        ...
        <div class="search-box">
          <div class="search-area">
            ...
            <i-input type="text" v-model="alitm" placeholder="请输入旺旺账号" class="search-input"/>
            <i-button type="primary" :loading="searchLoading" @click="searchOperation">查小号/查信誉</i-button>
          </div>
        </div>
        <div class="result-box fs-14" v-if="showResultBox">
          <p class="result-title clear" v-show="isLogin">
          ...
          <div v-else @click="toRegister" class="cursor-p">
        ...
        <div class="explain-box">
          <div class="explain-area">
            ...
            <p class="explain-content">1，注册时间<span class="point">小于30天</span>的，被认定为小号/新号，有可能是骗子，应当谨慎交易，免得赚几块钱换来一个中差评。
            </p>
            <p class="explain-content">2，该工具可以查询<span class="point">买家中差评数量</span>以及<span class="point">中差评</span>信息，如果该用户的比例超过一定的比例，则有可能是中差评师，要十分注意了。
            </p>
            <p class="explain-content">3，经常刷的小号，当信用点数<span class="point">超过20点</span>（常购物的买家大号除外），有可能这个号被淘宝列入黑名单，他再拍的东西有可能会被认为虚假交易，<span
              class="point">再刷会有降权风险</span>。</p>
    ...
    <!--快速注册登陆弹框-->
    <login-to-register-modal v-model="loginToRegisterModal" :on-success="loginSuccess"/>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      searchOperation() {
        const _this = this;
        if (!_this.alitm) {
          _this.$Message.warning('请输入要查询的旺旺名');
          return
        }
