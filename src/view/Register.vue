<template>
  <div class="register-ctt">
    ...
    <modal v-model="agreementShow" width="900">
      ...
      <div class="text-ct text-lf pl-20 pr-20" style="height:500px; overflow-y: scroll;">
        欢迎您使用白拿拿平台服务！
        <br>
        <br>
        ...
        <br>
        <br>
        当您按照注册页面提示填写信息、阅读并同意本协议且完成全部注册程序后，或您按照激活页面提示填写信息、阅读并同意本协议且完成全部激活程序后，或您以其他白拿拿允许的方式实际使用白拿拿平台服务时，即表示您已充分阅读、理解并接受本协议的全部内容，并与白拿拿平台达成协议。您承诺接受并遵守本协议的约定，
        届时您不应以未阅读本协议的内容或者未获得白拿拿对您问询的解答等理由，主张本协议无效，或要求撤销本协议。
        <br>
        <br>
        ...
        <br>
        (b)
        确保您在每个上网时段结束时，以正确步骤离开网站/服务。白拿拿不能也不会对因您未能遵守本款规定而发生的任何损失负责。您理解白拿拿对您的请求采取行动需要合理时间，白拿拿对在采取行动前已经产生的后果（包括但不限于您的任何损失）不承担任何责任。
        <br>
        <br>
        三、 白拿拿平台服务
        <br>
        ...
        4、对于您提供的资料及数据信息，您授予白拿拿及其关联公司独家的、全球通用的、永久的、免费的许可使用权利 (并有权在多个层面对该权利进行再授权)。此外，白拿拿及其关联公司有权(全部或部份地)
        使用、复制、修订、改写、发布、翻译、分发、执行和展示您的全部资料数据（包括但不限于注册资料、交易行为数据及全部展示于白拿拿平台的各类信息）或制作其派生作品，并以现在已知或日后开发的任何形式、媒体或技术，将上述信息纳入其它作品内。
        <br>
        <br>
      ...
      <div slot="footer" class="text-ct">
        <i-button type="error" size="large" @click="agreementShow = false">关闭</i-button>
      </div>
    </modal>
  </div>
</template>
    ...
    data() {
      ...
      const validateAgreeStrip = (rule, value, callback) => {
        if (!this.formCustom.agreeStrip) {
          return callback(new Error('请仔细阅读用户使用协议，并勾选！'));
        } else {
          callback();
        }
      };
      return {
        ...
        ruleCustom: {
          ...
          qqNumber: [
            {validator: validateQQNumber, trigger: 'blur'}
          ],
          validateCode: [
