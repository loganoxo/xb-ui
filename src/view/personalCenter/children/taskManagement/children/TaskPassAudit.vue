<template>
  <div class="mt-20">
              ...
              <tr>
                ...
                <td>
                  <div class="del-edit">
                    ...
                    <span class="cursor-p blue" v-if="item.status === 'pass_and_unclaimed'" @click="lookAtTemplate(item.reviewContent, item.reviewPictures)">查看评价范本</span>
                    ...
                    <span v-if="item.status !== 'order_num_waiting_audit' && item.status !== 'trial_report_waiting_confirm' && item.status !== 'trial_finished' && item.status !== 'pass_and_unclaimed'">------</span>
                  </div>
                </td>
              </tr>
              </tbody>
    ...
    <modal v-model="showTemplateModal" title="查看评价范本">
      ...
      <div class="mt-20">
        <span>评价晒图：</span>
        <img v-for="(item, index) in itemReviewAssign.reviewPictures" :key="index" class="border-radius-5 vtc-mid mr-10" :src="item" alt="评价范本图片" width="54" height="54">
      </div>
      <div slot="footer"></div>
    </modal>
  </div>
