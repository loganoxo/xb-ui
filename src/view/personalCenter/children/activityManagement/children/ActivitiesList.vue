<template>
  <div class="activity-management">
    ...
    <div class="list-sort clear">
      <i-select v-model="selectedStore" size="small" style="width:170px;" placeholder="全部店铺" :filterable="true"
                @on-change="filterStore" class="left mr-10">
        <i-option v-for="(item,index) in storeList" :key="index" :value="item">{{item}}</i-option>
      </i-select>
      <i-select v-model="activityCategory" size="small" style="width:108px" class="left mr-10" placeholder="全部类型活动"
                @on-change="changeActivityCategory">
        <i-option v-for="item in activityTypeList" :value="item.value" :key="item.value">{{ item.label }}</i-option>
      </i-select>
      <button-group class="left">
        <i-button :class="[sortList.select === item.sortField ? 'active' : '']" size="small"
        ...
        <template v-for="item in taskList">
          ...
          <tr>
            ...
            <td v-if="item.settlementStatus === 'settlement_finished'">
              ...
              {{((item.promotionExpensesNeed > 0 ? item.promotionExpensesNeed : 0) / item.taskCount *
              item.showkerApplySuccessCount / 100).toFixed(2)}} /
            ...
            <td v-if="item.settlementStatus !== 'settlement_finished'">
              （ {{(item.totalMarginNeed / 100).toFixed(2)}} / {{((item.promotionExpensesNeed > 0 ?
              item.promotionExpensesNeed : 0) / 100).toFixed(2)}} / {{((item.vasFeeNeed + item.tagVasFeeNeed) /
              100).toFixed(2)}}）
    ...
    <modal v-model="directSettlementSuccess" width="360">
      ...
      <div slot="footer" class="clear">
        ...
        <i-button class="right mr-30" type="primary" size="large" @click="directSettlementSuccess = false">我知道了
        </i-button>
      </div>
    </modal>
    <!--结算成功弹框-结算有返款-->
    ...
    <modal v-model="billDetailsModel" width="420">
      ...
      <div>
        ...
        <p v-if="taskSettlementDetailInfo.taskSettlement" class="mt-5">
          结算备注：本次结算名额：{{taskSettlementDetailInfo.refundTaskCount}} ；</p>
    ...
    <modal v-model="isTaskOverdueModel" width="420" :mask-closable="false" :closable="false">
      ...
      <div slot="footer" class="text-ct">
        <i-button type="primary" size="large" long :loading="taskOverdueLoading" @click="isTaskOverdueClose">关闭活动
        </i-button>
      </div>
    </modal>
    <!--支付保证金弹框-->
    <div class="pay-model" v-if="showPayModel">
      <pay-model ref="payModelRef" :orderMoney="needPayMoney" @confirmPayment="confirmPayment" :isShowUpgradeVIP="true"
                 ...
                 @change="redEnvelopesState = arguments[0]"
        ...
        <div slot="noBalance" class="title-tip">
          ...
          </span>还需充值<strong class="size-color3">{{needPayMoneyText}}</strong>元 <span
          @click="isShowAliPayTip = true">【<span class="blue cursor-p">支付宝手续费</span>】</span>
        </div>
        <div slot="isBalance" class="title-tip">
          <icon color="#FF2424" size="18px" type="md-alert"/>
          <span class="ml-10">您本次需要支付金额为 <span
            class="size-color3">{{(orderMoneyAfter / 100).toFixed(2)}}</span> 元。</span>
        </div>
      </pay-model>
    </div>
    <!--支付宝手续费说明弹框-->
    ...
    <modal v-model="deleteFirstTaskModal" width="360">
      ...
      <div slot="footer" class="text-ct">
        <i-button class="mr-40 pr-40 pl-40" type="primary" size="large" :loading="speedUpLoading" @click="confirmClose">
          确认
        </i-button>
        <i-button class="ml-40 pr-40 pl-40" type="primary" size="large" @click="deleteFirstTaskModal = false">取消
        </i-button>
      </div>
    </modal>
    <!--任务或者活动结算-->
    <modal v-model="showSettlementModal">
      ...
      <div slot="footer" class="clear">
        <i-button class="task-settlement-btn left width-pct-39 ml-20 bg-main-color cl-fff"
                  :disabled="!settlementData.taskSettle"
                  @click="taskSettlement(settlementData.id,settlementData.taskName)">任务结算
        </i-button>
        <i-button class="activity-settlement-btn right width-pct-39 mr-20 bg-main-color cl-fff"
                  :disabled="!settlementData.activitySettle"
                  @click="activitySettlement(settlementData.id, settlementData.number)">活动结算
        </i-button>
      </div>
    </modal>
    <!--修改活动备注弹窗-->
